...
public class ReviewersController {
    ...
    @PutMapping("/update")
    public RespBean changeInEnable(int id) {
        int result = reviewersService.changeInEnable(id);
        if (result == 1) {
    ...
    @PostMapping("/add")
    public RespBean addReviewer(@RequestBody Reviewers reviewers) {
        int result = reviewersService.addReviewer(reviewers);
        if (result == 1) {
            return RespBean.ok("创建评审账号成功");
        }
        return RespBean.error("创建评审账号失败");
    }
}
