...
public class UsersController {
    ...
    public RespBean login(String phone, String password, String ID) {
        if (ID.equals("学生")) {
            ...
            if (users != null) {
                int result = usersService.checkUser(phone, password);
                if (result == 1) {
                    return RespBean.error("您的账户已经被禁用");
                } else {
    ...
    public RespBean getMyJoinCompetition(int id) {
        return usersService.getMyJoinCompetition(id);
    }

    @PutMapping("/update")
    public RespBean changeInEnable(int id) {
        int result = usersService.changeInEnable(id);
        if (result == 1) {
